...
static DescriptorSet *g_DescriptorSet;
...
static void ImGui_ImplShitRenderer_SetupRenderState(ImDrawData *draw_data, Pipeline *pipeline, CommandBuffer *command_buffer, ImGui_ImplShitRendererH_FrameRenderBuffers *rb, int fb_width, int fb_height)
{
	...
	// Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
	{
		...
		if (displaySize.x != draw_data->DisplaySize.x || displaySize.y != draw_data->DisplaySize.y)
		{
			//float scale[2];
			...
			if ((g_ShitRendererInitInfo.renderSystem->GetCreateInfo()->version & RendererVersion::TypeBitmask) == RendererVersion::GL)
			{
...
void ImGui_ImplShitRenderer_RenderDrawData(ImDrawData *draw_data, CommandBuffer *command_buffer, Pipeline *pipeline)
{
	...
	ImGui_ImplShitRenderer_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width, fb_height);
...
{
	auto rendererType = g_ShitRendererInitInfo.renderSystem->GetCreateInfo()->version & RendererVersion::TypeBitmask;
	...
	if (!g_DescriptorSet)
	{
		...
		g_DescriptorPool->Allocate(
			DescriptorSetAllocateInfo{
				{g_DescriptorSetLayout}},
			descriptorSets);
		g_DescriptorSet = descriptorSets[0];
	}
...
void ImGui_ImplShitRenderer_RecordCommandBuffer(uint32_t imageIndex, RenderPass *renderPass, uint32_t subpass, Framebuffer *pFrameBuffer)
{
	...
	ImGui_ImplShitRenderer_RenderDrawData(draw_data, cmdBuffer, g_Pipeline);
	cmdBuffer->End();
}
