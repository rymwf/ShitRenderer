...
static auto vertShaderPath0 = "Abuffer/write.vert";
static auto fragShaderPath0 = "Abuffer/write.frag";
static auto vertShaderPath1 = "Abuffer/read.vert";
static auto fragShaderPath1 = "Abuffer/read.frag";
...
class Hello : public AppBase {
    ...
    std::vector<Shit::Buffer *> atomicBuffers;
    std::vector<Shit::Buffer *> listBuffers;
    std::vector<Shit::BufferView *> listBufferViews;
    std::vector<Shit::Image *> headImages;
    std::vector<Shit::ImageView *> headImageViews;
...
public:
    void loadAssets() {
        ...
        for (uint32_t i = 0; i < frameCount; ++i) {
            atomicBuffers.emplace_back(_device->Create(
                ...
                0));
            auto listBuffer = listBuffers.emplace_back(
                _device->Create(Shit::BufferCreateInfo{{},
                                                       listBufferSize,
                                                       Shit::BufferUsageFlagBits::STORAGE_TEXEL_BUFFER_BIT,
                                                       Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT}));
            listBufferViews.emplace_back(_device->Create(
                Shit::BufferViewCreateInfo{listBuffer, Shit::Format::R32G32B32A32_UINT, 0, listBufferSize}));
            auto image = headImages.emplace_back(_device->Create(
    ...
    void createSkyboxRenderTarget() {
        ...
        for (uint32_t i = 0; i < imageCount; ++i) {
            auto pImageView = skyboxRenderTarget.imageViews.emplace_back(_swapchain->GetImageViewByIndex(i));
    ...
    void preparePipelineInfos() {
        auto frameCount = getFramesInFlight();
        {
            Shit::DescriptorSetLayoutBinding bindings[]{
                ...
                {1, Shit::DescriptorType::STORAGE_TEXEL_BUFFER, 1, Shit::ShaderStageFlagBits::FRAGMENT_BIT},
                {2, Shit::DescriptorType::STORAGE_BUFFER, 1, Shit::ShaderStageFlagBits::FRAGMENT_BIT},
            };
            auto setLayout =
    ...
    void updateDescriptorSets() {
        ...
        std::vector<Shit::DescriptorBufferInfo> buffersInfo(frameCount);
        std::vector<Shit::DescriptorImageInfo> imagesInfo(frameCount);
        std::vector<Shit::WriteDescriptorSet> writes;

        for (uint32_t i = 0; i < frameCount; ++i) {
            imagesInfo[i] = {0, headImageViews[i], Shit::ImageLayout::GENERAL};
            buffersInfo[i] = {atomicBuffers[i], 0, atomicBuffers[i]->GetCreateInfoPtr()->size};

            writes.emplace_back(Shit::WriteDescriptorSet{pipelineInfo0.descriptorSets[i], 0, 0, 1,
                                                         Shit::DescriptorType::STORAGE_IMAGE, &imagesInfo[i]});
            writes.emplace_back(Shit::WriteDescriptorSet{pipelineInfo0.descriptorSets[i], 1, 0, 1,
                                                         Shit::DescriptorType::STORAGE_TEXEL_BUFFER, 0, 0,
                                                         &listBufferViews[i]});
            writes.emplace_back(Shit::WriteDescriptorSet{pipelineInfo0.descriptorSets[i], 2, 0, 1,
                                                         Shit::DescriptorType::STORAGE_BUFFER, 0, &buffersInfo[i]});
    ...
    void recordDefaultCommandBuffer(uint32_t imageIndex) override {
        ...
        cmdBuffer->FillBuffer(
            {atomicBuffers[_currentFrame], 0, atomicBuffers[_currentFrame]->GetCreateInfoPtr()->size, 0});
        Shit::ImageSubresourceRange range{Shit::ImageAspectFlagBits::COLOR_BIT, 0, 1, 0, 1};
        cmdBuffer->ClearColorImage({headImages[_currentFrame], Shit::ImageLayout::GENERAL,
                                    Shit::ClearColorValueUint32{0xFFFFFFFF}, 1, &range});
    ...
    void recreateSwapchainListener() {
        ...
        auto ext = _swapchain->GetCreateInfoPtr()->imageExtent;
        for (uint32_t i = 0; i < frameCount; ++i) {
            _device->Destroy(headImages[i]);
            _device->Destroy(headImageViews[i]);

            headImages[i] = _device->Create(Shit::ImageCreateInfo{{},
                                                                  Shit::ImageType::TYPE_2D,
                                                                  Shit::Format::R32_UINT,
                                                                  {ext.width, ext.height, 1},
                                                                  1,
                                            ...
                                            Shit::ImageAspectFlagBits::COLOR_BIT, -1);
            headImageViews[i] =
                _device->Create(Shit::ImageViewCreateInfo{headImages[i],
                                                          Shit::ImageViewType::TYPE_2D,
                                                          Shit::Format::R32_UINT,
                                                          {},
                                                          {Shit::ImageAspectFlagBits::COLOR_BIT, 0, 1, 0, 1}});
