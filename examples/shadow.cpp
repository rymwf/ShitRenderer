...
static auto vertShaderPath = "shadow/display.vert";
static auto fragShaderPath = "shadow/display.frag";
static auto showTexVertShaderPath = "shadow/showTex.vert";
static auto showTexFragShaderPath = "shadow/showTex.frag";
...
float depthBiasSlopFactor = 3.;
float depthBiasClamp = 0.;

class LightController : public Behaviour {
    void prepareImpl() override { getParentTransform()->translate({-12., 3., 3.})->yaw(glm::radians(-90.f)); }
    void updatePerFrameImpl(float frameDtMs) override {
        // getParentTransform()->yaw(glm::radians(frameDtMs / 1000 *
        // _anglePerSecond));
    }
...
public:
    void loadAssets() {
        ...
        light = _gameObjects.emplace_back(std::make_unique<GameObject>())->addComponent<Light>(LIGHT_SPOT);
    ...
    void writeDescriptorSets() {
        ...
        // write
        Shit::DescriptorImageInfo imageInfo[]{
            {sampler, shadowRenderTarget.imageViews[0], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
            {shadowSamplerLinear, shadowRenderTarget.imageViews[0], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
        };
        ...
        for (int i = 0; i < count; ++i) {
            ...
            writeSets.emplace_back(Shit::WriteDescriptorSet{
                pipelineInfo.descriptorSets[i], 8, 0, 1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, &imageInfo[0]});
            writeSets.emplace_back(Shit::WriteDescriptorSet{
                pipelineInfo.descriptorSets[i], 9, 0, 1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, &imageInfo[1]});
        };
    ...
    void updateUBODisplayBuffer() {
        ...
        memcpy(data, &uboDisplay, sizeof(UBODisplay));
    ...
    void setupImGui() {
        ...
        ImGui::Separator();
        {
            ...
            // }
            static auto lightPos = light->getParentTransform()->getLocalTranslation();
            static glm::vec3 lightRotation =
                glm::degrees(glm::eulerAngles(light->getParentTransform()->getLocalRotation()));
            if (ImGui::DragFloat3("lightPos", (float *)&lightPos, 0.1f)) {
                light->getParentTransform()->setLocalTranslation(lightPos);
            }
            if (ImGui::DragFloat3("lightRotation", (float *)&lightRotation, 0.1f)) {
                light->getParentTransform()->setLocalRotation(glm::quat(glm::radians(lightRotation)));
            }
            static float umbraAngle = glm::degrees(light->getSpotUmbraAngle());
            static float penumbraAngle = glm::degrees(light->getSpotPenumbraAngle());
            static bool lightUpdate = false;
            lightUpdate |= ImGui::SliderFloat("spot umbra", &umbraAngle, penumbraAngle, 90.f);
            lightUpdate |= ImGui::SliderFloat("spot peumbra", &penumbraAngle, 0., umbraAngle);
            if (lightUpdate) {
                light->setSpotLight(glm::radians(umbraAngle), glm::radians(penumbraAngle));
                lightUpdate = false;
            }
            ...
            updateUBOBuffer |= ImGui::RadioButton("PCF_HW", &renderMode, 4);
            updateUBOBuffer |= ImGui::RadioButton("PCSS_HW", &renderMode, 5);
            updateUBOBuffer |= ImGui::RadioButton("VSM", &renderMode, 6);

            updateUBOBuffer |= ImGui::SliderFloat("normalBiasFactor", &uboDisplay.normalBiasFactor, 0.f, 0.1f);
            updateUBOBuffer |= ImGui::SliderFloat("shadowFilterRadius", &uboDisplay.shadowFilterRadius, 1.f, 10.f);
            if (updateUBOBuffer) {
                uboDisplay.renderMode = shadowOnly ? -renderMode : renderMode;
