...
static auto vertShaderPath = "shadowcascade/display.vert";
static auto fragShaderPath = "shadowcascade/display.frag";
static auto showTexVertShaderPath = "shadowcascade/showTex.vert";
static auto showTexFragShaderPath = "shadowcascade/showTex.frag";
static auto shadowVertShaderPath = "shadowcascade/shadow.vert";
static auto shadowGeomShaderPath = "shadowcascade/shadow.geom";
static auto shadowFragShaderPath = "shadowcascade/shadow.frag";

static const char *testModelPaths[]{
    SHIT_SOURCE_DIR "/assets/models/obj/terrain/terrain.obj",
    SHIT_SOURCE_DIR "/assets/models/obj/oaktree/oaktree.obj",
    SHIT_SOURCE_DIR "/assets/models/obj/cylinder.obj",
};
...
class LightController : public Behaviour {
    void prepareImpl() override { getParentTransform()->pitch(glm::radians(-30.f)); }
...
public:
    void loadAssets() {
        auto terrian = static_cast<Model *>(_modelManager->create(testModelPaths[0]));
        terrian->load();
        auto tree = static_cast<Model *>(_modelManager->create(testModelPaths[1]));
        tree->load();
        auto cylinder = static_cast<Model *>(_modelManager->create(testModelPaths[2]));
        cylinder->load();

        // add terrian
        auto terrianObj = _gameObjects.emplace_back(std::make_unique<GameObject>()).get();
        terrianObj->getComponent<Transform>()->scale({10, 10, 10});

        for (auto it = terrian->meshBegin(), end = terrian->meshEnd(); it != end; ++it) {
            terrianObj->addChild()->addComponent<MeshRenderer>(it->get());
        }
        terrianObj->prepare();

        // add trees
        int column = 6, row = 6, interval = 10, i = 0, j = 0;
        for (; j < row; ++j) {
            for (i = 0; i < column; ++i) {
                auto treeObj = _gameObjects.emplace_back(std::make_unique<GameObject>()).get();
                treeObj->getComponent<Transform>()->translate(
                    {float(2 * i - column + 1) / 2 * interval, 1, float(2 * j - row + 1) / 2 * interval});
                Model *pModel = tree;
                if (i % 2) {
                    pModel = cylinder;
                    treeObj->getComponent<Transform>()->translate({0, 1, 0})->scale({1, 5, 1});
                } else {
                    treeObj->getComponent<Transform>()->scale({3, 3, 3});
                }

                for (auto it = pModel->meshBegin(), end = pModel->meshEnd(); it != end; ++it) {
                    treeObj->addChild()->addComponent<MeshRenderer>(it->get());
                }
                treeObj->prepare();
            }
        }
        ...
        _editorCamera->setFarPlane(100.f);
        _editorCamera->getParentTransform()->updateSignal.Connect(
            std::bind(&Hello::cameraListener, this, std::placeholders::_1));
        _editorCamera->updateSignal.Connect(std::bind(&Hello::cameraListener, this, std::placeholders::_1));

        float splitRatios[]{0.2, 0.4, 0.6};
        // float splitRatios[]{0.5};
        _editorCamera->setCascadeSplitRatios(splitRatios);
        std::vector<glm::vec3> vertices;
        for (int i = 0; i < _editorCamera->getCascadeNum(); ++i) {
            auto &&a = _editorCamera->getFrustumVertices(i);
            for (auto &&e : a) vertices.emplace_back(e);
        }
        light->setCascadeFrustumVertices(vertices);
        cascadeNum = _editorCamera->getCascadeNum();
    }
    ...
    void prepareShadowRenderTarget() {
        ...
        auto depthImageViewCI =
            Shit::ImageViewCreateInfo{nullptr,
                                      Shit::ImageViewType::TYPE_2D_ARRAY,
                                      depthFormat,
                                      {},
                                      {Shit::ImageAspectFlagBits::DEPTH_BIT, 0, 1, 0, (uint32_t)cascadeNum}};
        auto count = 1;
    ...
    void writeShowTexDescriptorSets() {
        ...
        Shit::DescriptorImageInfo imageInfo{sampler, shadowRenderTarget.imageViews[0],
                                            Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
        std::vector<Shit::WriteDescriptorSet> writeSets{
    ...
    void cameraListener(Component const *pComp) {
        std::vector<glm::vec3> vertices;
        for (int i = 0; i < _editorCamera->getCascadeNum(); ++i) {
            auto &&a = _editorCamera->getFrustumVertices(i);
            for (auto &&e : a) vertices.emplace_back(e);
        ...
        light->setCascadeFrustumVertices(vertices);
        if (viewMode == 1) _needUpdateDefaultCommandBuffer = true;
    }
    ...
    void setupImGui() {
        ...
        ImGui::Separator();
        {
            ...
            if (viewMode == 0) {
                ...
                updateUBOBuffer |= ImGui::Checkbox("showCascade", (bool *)&uboDisplayData.showCascade);
                ...
                updateUBOBuffer |=
                    ImGui::SliderFloat("shadowFilterRadius", &uboDisplayData.shadowFilterRadius, 1.f, 10.f);
                if (updateUBOBuffer) {
            ...
            if (viewMode == 1) {
                _needUpdateDefaultCommandBuffer |=
                    ImGui::SliderInt("texLayer", &showTexVar.texLayer, 0, cascadeNum - 1);
            }
        }
        ImGui::End();
    }
