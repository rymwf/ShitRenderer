...
Shit::RendererVersion g_RendererVersion{Shit::RendererVersion::GL};
...
//		if (s == "GL")
//		{
//			g_RendererVersion = RendererVersion::GL;
//		}
//		else if (s == "VK")
//		{
//			g_RendererVersion = RendererVersion::VULKAN;
//		}
//		else
//		{
	...
	ImageLayout dstFinalLayout)
{
	...
	std::string compShaderPath = buildShaderPath(compShaderNameGenerateCubemap, g_RendererVersion);
	...
	ImageLayout dstFinalLayout)
{
	...
	std::string llmShaderPath = buildShaderPath(llmCompShaderName, g_RendererVersion);
	std::string shShaderPath = buildShaderPath(shCompShaderName, g_RendererVersion);
	...
	ImageLayout dstFinalLayout)
{
	...
	std::string prefilterShaderPath = buildShaderPath(prefilterShaderName, g_RendererVersion);
	...
	for (auto e : pipelines)
		pDevice->Destroy(e);
	pDevice->Destroy(prefilterShader);
}

void generateEnvBRDFMap(Device *pDevice, Image *&envBRDFImage, ImageView *&envBRDFImageView, ImageLayout finalLayout)
{
	...
	auto shaderPath = buildShaderPath(shaderName, g_RendererVersion);
...
Image *generateImage2D(
	Device *pDevice,
	const char *spirvShaderName,
	unsigned int width,
	unsigned int height,
	ImageLayout finalLayout)
{
	Image *pOutImage = pDevice->Create(
		ImageCreateInfo{
			{},
			ImageType::TYPE_2D,
			ShitFormat::RGBA32_SFLOAT,
			...
			ImageLayout::GENERAL},
		nullptr);
	ImageView *pOutImageView = pDevice->Create(
		ImageViewCreateInfo{
			pOutImage,
			ImageViewType::TYPE_2D,
			pOutImage->GetCreateInfoPtr()->format,
			{},
			{0, 1, 0, 1}});
	//===================================
	//config descriptor
	...
	//create pipeline
	auto shaderPath = buildShaderPath(spirvShaderName, g_RendererVersion);
	auto shaderSource = readFile(shaderPath.c_str());
	auto shader = pDevice->Create(ShaderCreateInfo{shaderSource.size(), shaderSource.data()});
	...
	auto pipeline = pDevice->Create(ComputePipelineCreateInfo{
		PipelineShaderStageCreateInfo{
			...
			shader,
			"main",
			SpecializationInfo{constantIDs, constantValues}},
		pipelineLayout});
	...
	std::vector<WriteDescriptorSet> writes{
		{descriptorSets[0],
		 ...
		 DescriptorType::STORAGE_IMAGE,
		 std::vector<DescriptorImageInfo>{
			 {nullptr, pOutImageView, ImageLayout::GENERAL},
		 }},
	};
		...
		[&](CommandBuffer *pCommandBuffer) {
			...
			pCommandBuffer->BindPipeline(
				BindPipelineInfo{
					PipelineBindPoint::COMPUTE,
					pipeline});
			pCommandBuffer->Dispatch(DispatchInfo{height, 1, 1});
			//convert image to dst image layout
			if (finalLayout != ImageLayout::GENERAL)
			{
				ImageMemoryBarrier barrier{
...
Image *generateEqirectangularProceduralSkybox(
	Device *pDevice,
	...
	ImageLayout finalLayout)
{
	...
	static Image *pOpticalDepthImage = generateImage2D(pDevice, "opticalDepth.comp.spv", 64, 64, ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	static ImageView *pOpticalDepthImageView = pDevice->Create(
		ImageViewCreateInfo{
			pOpticalDepthImage,
			ImageViewType::TYPE_2D,
			pOpticalDepthImage->GetCreateInfoPtr()->format,
			{},
			{0, 1, 0, 1}});

	//takeScreenshot(pDevice, pOpticalDepthImage, ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	//std::this_thread::sleep_for(std::chrono::seconds(1));
	...
	//allocate compute descriptor set
	DescriptorSetAllocateInfo allocInfo{std::vector(1, setLayout)};
	std::vector<DescriptorSet *> descriptorSets;
	descriptorPool->Allocate(allocInfo, descriptorSets);
	...
	PipelineLayout *pipelineLayout = pDevice->Create(PipelineLayoutCreateInfo{{setLayout}});
	//create pipeline
	auto shaderPath = buildShaderPath("skyboxProcedural2D.comp.spv", g_RendererVersion);
	auto shaderSource = readFile(shaderPath.c_str());
	auto shader = pDevice->Create(ShaderCreateInfo{shaderSource.size(), shaderSource.data()});
	...
	std::vector<WriteDescriptorSet> writes{
		{descriptorSets[0],
		 ...
		 DescriptorType::COMBINED_IMAGE_SAMPLER,
		 std::vector<DescriptorImageInfo>{
			 {linearSampler, pOpticalDepthImageView, ImageLayout::SHADER_READ_ONLY_OPTIMAL},
		 }},
		{descriptorSets[0],
		 1,
		 0,
	...
	executeOneTimeCommands(pDevice, QueueFlagBits::COMPUTE_BIT, 0, [&](CommandBuffer *pCommandBuffer) {
		...
		pCommandBuffer->BindPipeline(
			BindPipelineInfo{
				PipelineBindPoint::COMPUTE,
				pipeline});
		pCommandBuffer->Dispatch(DispatchInfo{
			height, 1, 1});
	});
	pDevice->Destroy(linearSampler);
	pDevice->Destroy(descriptorPool);
	pDevice->Destroy(setLayout);
	pDevice->Destroy(pOutImageView);
	pOutImage->GenerateMipmaps(Filter::LINEAR, ImageLayout::GENERAL, finalLayout);
	return pOutImage;
}