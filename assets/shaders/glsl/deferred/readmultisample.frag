...
#define SAMPLE_FETCH(a,b,c) subpassLoad(a,c)
...
layout (INPUT_ATTACHMENT_INDEX(0) binding = 1) uniform SAMPLE_INPUT_MS inputPos;
layout (INPUT_ATTACHMENT_INDEX(1) binding = 2) uniform SAMPLE_INPUT_MS inputAlbedo;
layout (INPUT_ATTACHMENT_INDEX(2) binding = 3) uniform SAMPLE_INPUT_MS inputN;
layout (INPUT_ATTACHMENT_INDEX(3) binding = 4) uniform SAMPLE_INPUT_MS inputMRA;
layout (INPUT_ATTACHMENT_INDEX(4) binding = 5) uniform SAMPLE_INPUT_MS inputEmission;
...
//=====================================
vec4 resolve(SAMPLE_INPUT_MS inputAttachment)
{
	ivec2 uv=ivec2(gl_FragCoord.xy);
	vec4 ret=vec4(0);
	for(int i=0;i<sampleCount;++i)
	{
		ret+=SAMPLE_FETCH(inputAttachment,uv,i);
	}
	ret/=sampleCount;
	return ret;
}
