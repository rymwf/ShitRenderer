...
void main()
{
	...
	vec3 emission=uboMaterial.emission;
	...
	if(uboMaterial.emissive_tex_index>=0)
		emission=texture(textures[uboMaterial.emissive_tex_index],fs_in.texcoord).rgb;
	if(uboMaterial.ambient_tex_index>=0)
		ao=texture(textures[uboMaterial.ambient_tex_index],fs_in.texcoord).r;
