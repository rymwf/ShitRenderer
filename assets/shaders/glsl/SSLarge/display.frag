...
void main()
{
	...
	vec3 f_baseLayer=f_spec+f_ms;
	vec3 f_baseLayer_ambient=f_spec_ambient+f_ms_ambient;

	vec3 Lo=vec3(0);
	Lo=f_baseLayer*irradiance+f_baseLayer_ambient*ambientIrradiance;

	float d=(1-ao.y+uPc.thicknessOffset)*uPc.thicknessScale;
	vec3 f_trans_ambient=exp(-d)*albedo.rgb;
	Lo+=ambientIrradiance*f_trans_ambient;

	vec3 LT=L+N*uPc.fLTDistortion;
	float f_back=clamp(pow(clamp(max(dot(V,-LT),0),0,1),uPc.fLTPower)*uPc.fLTScale,0,1);
	vec3 fLT=uboLight.color*uboLight.radiance*PI*f_back*ao.y*albedo.rgb;
	Lo+=fLT;

	float f_front=clamp((NdotL+uPc.kwrap)/(1+uPc.kwrap),0,1);
	Lo+=uboLight.color*uboLight.radiance*PI*f_trans_ambient*f_front;
