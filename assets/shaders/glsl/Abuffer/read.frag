...
layout(binding=0,r32ui) uniform readonly uimage2D listHeaderImage;
...
{
	uint current=imageLoad(listHeaderImage,ivec2(gl_FragCoord.xy)).x;
	int fragCount=0;
	uvec4 item;
	while(current!=0xFFFFFFFF&&fragCount<MAX_FRAGMENTS)
	{
		item=imageLoad(listBuffer,int(current));
		current=item.x;
