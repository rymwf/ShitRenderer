...
layout(SET(4)binding=0,r32ui)uniform uimage2D listHeaderImage;
layout(SET(4)binding=1,rgba32ui)uniform writeonly uimageBuffer listBuffer;
layout(SET(4)binding=2)buffer atomicBuffer{
	uint fragmentIndex;
};
...
void main()
{
	...
	uint index=atomicAdd(fragmentIndex,1);
	uvec4 item=uvec4(0);
	item.x=imageAtomicExchange(listHeaderImage,ivec2(gl_FragCoord.xy),index);
	item.y=packUnorm4x8(vec4(col,.7));
	item.z=floatBitsToUint(gl_FragCoord.z);
	imageStore(listBuffer,int(index),item);
}