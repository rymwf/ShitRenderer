...
layout (location=1) out float gs_depth;
...
void main()
{
	...
	for(int i=0;i<count;++i)
	{
		...
		for(int j=0;j<3;++j)
		{
			gl_Position=uboLight.PV[i*2]*uboLight.PV[i*2+1]*gl_in[j].gl_Position;
			gs_depth=length(gl_in[j].gl_Position.xyz-uboLight.transformMatrix[3].xyz)/uboLight.rmax;
			gs_depth+=0.001;
