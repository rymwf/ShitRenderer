...
void main()
{
	vec3 V= normalize(camera[VIEWID].eyePos-fs_in.position);
	//vec3 
	//
	vec3 L=vec3(1,0,0);
	...
	if(bump_tex_index>=0)
	{
	 	N+=TBN*normalize(texture(textures[bump_tex_index],fs_in.texcoord).xyz*2-1);
		N=normalize(N);
	}
	...
	if(diffuse_tex_index>=0)
		albedo*=texture(textures[diffuse_tex_index],fs_in.texcoord).rgb;
	if(emissive_tex_index>=0)
		emissiveColor=texture(textures[emissive_tex_index],fs_in.texcoord).rgb;
